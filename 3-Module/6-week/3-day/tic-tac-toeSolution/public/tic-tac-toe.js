...
function renderBoard(squareValues) {
    ...
    for (let i = 0; i < 9; i += 1) {
        ...
        if (squareValues[i] !== "") {
            ...
            img.src = `https://assets.aaonline.io/Module-DOM-API/formative-project-tic-tac-toe/player-${squareValues[
                i
            ].toLowerCase()}.svg`;
        } else if (img) {
...
function renderNewGameBtn(gameStatus) {
    ...
    document.getElementById("new-game").disabled = !gameStatus;
}

function renderGiveUpBtn(gameStatus) {
    // disable button if game is over
    document.getElementById("give-up").disabled = !!gameStatus;
}
