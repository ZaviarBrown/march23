...
const cookieFinder = (key) => {
    const cookie = document.cookie.split("; ").find((el) => el.startsWith(key));
...
function storeTheme(themeName) {
    // document.cookie = `theme=${themeName};`; // Phase 1
    document.cookie = `theme=${themeName}; max-age=30`; // Phase 2
}

// For restoring theme from cookies, if selected by the user in the past
function restoreTheme() {
    const theme = cookieFinder("theme");
