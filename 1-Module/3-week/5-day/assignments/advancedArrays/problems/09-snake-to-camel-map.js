...
function snakeToCamel(str) {
    // Your code here
    let words = str.split("_"); //pull out _ and get back an array of words
    let fixed = words.map(word => word[0].toUpperCase() + word.slice(1).toLowerCase()); 
    // fix casing for words (fourth example has some wonky casing)
    return fixed.join("");
}
