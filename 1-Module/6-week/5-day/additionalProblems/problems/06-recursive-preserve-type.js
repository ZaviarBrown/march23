...
//! Recursive default copyArr version
function recursivePreserveType(array) {
    const inner = (type, finalArr = [], copyArr = [...array]) => {
        // Base case
        if (!copyArr.length) {
            return finalArr;
        }
        ...
        // Recursive Step
        const firstVal = copyArr.shift();

        if (typeof firstVal === type) {
            finalArr.push(firstVal);
        }

        // Recursive Case

        return inner(type, finalArr, copyArr);
    };

    return inner;
}

// //! Recursive index version
// function recursivePreserveType(array) {
