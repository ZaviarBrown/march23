...
//! with for in loop
const highestValueKey = (countObj) => {
    let highestKey = "";
    let highestNum = 0;

    for (let key in countObj) {
        const currNum = countObj[key];
...
const countVowels = (word, counterObj) => {
...
const highestValueKey = (countObj) => {
    ...
    return keys.reduce((highestKey, nextKey) => {
        const currNum = countObj[highestKey];
        const nextNum = countObj[nextKey];
...
const mostFrequentVowel = function (words, counter = {}) {
    // Base Case
    if (!words.length) {
        return highestValueKey(counter);
    }
