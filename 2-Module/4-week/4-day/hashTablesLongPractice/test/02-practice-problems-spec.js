...
const [
    ...
    twoSum,
    wordPattern,
] = require("../phases/02-practice-problems");
...
describe("Phase 2 - Hash table practice problems", function () {
    it("`anagrams` tests if two strings are anagrams of each other", function () {
        ...
        // expect(anagrams("hhhah", "hhahh")).to.be.true;
        // expect(anagrams("aaaah", "ah")).to.be.false;
        // expect(anagrams("aah", "ahh")).to.be.false;
        // expect(anagrams("ah", "ah")).to.be.true;
    });
    ...
    it("`commonElements` returns the common integers in both arrays", function () {
        ...
        expect(commonElements([2, 4, 6], [5, 4, 3, 2, 1, 4])).to.include.members([
            2, 4,
        ]);
    });
    ...
    it("`commonElements` runs in O(n) time", function () {
        expect(commonElements([1, 2, 3], [3, 4, 5])).to.include.members([3]);
        expect(commonElements([2, 4, 6], [5, 4, 3, 2, 1])).to.include.members([
            2, 4,
        ]);
