...
describe("getNeighbors", () => {
    ...
    it("can return adjacent neighbors that contain a 1", () => {
        expect(getNeighbors(1, 1, matrix)).to.have.deep.members([
            [0, 1],
            [1, 2],
            [2, 1],
        ]);
    ...
    it("will return neighbors from 0s", () => {
        expect(getNeighbors(1, 0, matrix)).to.have.deep.members([
            [0, 0],
            [1, 1],
        ]);
    });
...
describe("islandSize", () => {
    ...
    // it("should not be dumb but it kind of is", () => {
    //     expect(islandSize(1, 0, matrix)).to.equal(7);
    // });
});
