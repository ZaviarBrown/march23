...
app.post("/musicians/:musicianId/instruments", async (req, res, next) => {
    ...
    const chosenMusician = await Musician.findByPk(musicianId);

    chosenMusician.addInstruments(instrumentIds);

    return res.json({
        message: `Associated ${chosenMusician.firstName} with instruments ${instrumentIds}.`,
    });
});
