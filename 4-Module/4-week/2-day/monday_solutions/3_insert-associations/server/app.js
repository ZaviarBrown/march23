...
app.post("/bands/:bandId/musicians", async (req, res, next) => {
    const currBand = await Band.findByPk(req.params.bandId);
    const newMusician = await currBand.createMusician(req.body);

    return res.json({
        message: `Created new musician for band ${currBand.name}.`,
...
app.post("/musicians/:musicianId/instruments", async (req, res, next) => {
    ...
    const musicianInstance = await Musician.findByPk(musicianId);

    const instrumentInstances = [];
    for (const id of instrumentIds) {
        const currInstrument = await Instrument.findByPk(id);
        instrumentInstances.push(currInstrument);
    }

    // try {
    //     await musicianInstance.addInstruments("blueberry");
    // } catch (e) {
    //     console.log(e);
    // }

    musicianInstance.addInstruments(instrumentIds);

    return res.json({
        message: `Associated ${musicianInstance.firstName} with instruments ${instrumentIds}.`,
    });
});
