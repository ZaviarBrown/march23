...
app.get("/bands-eager", async (req, res, next) => {
    const payload = await Band.findAll({
        include: { model: Musician },
        // include: Musician, //? Also works to solve this problem
...
// //! Lazy loading better in this case
// app.get("/bands-lazy/:id", async (req, res, next) => {
//     const band = await Band.findByPk(req.params.id);

//     if (band.name === "Pumpkin kids") {
...
//     include: { model: Musician },
//     order: [[Musician, "firstName"]],
//     include: { model: Musician, attributes: ["firstName"] }, //!
//     include: [Musician, Instruments], //!
//     include: [{ model: Musician }, { model: Instruments }], //!
// });

//     if ((payload.name = "Pumpkin kids")) {
//         delete payload.Musicians;
