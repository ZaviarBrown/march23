...
export const getReportsThunk = () => async (dispatch) => {
    const data = await fetch("/api/reports").then((res) => res.json());
    dispatch(loadReports(data));
};

export const getSingleReportThunk = (id) => async (dispatch) => {
    const data = await fetch(`/api/reports/${id}`).then((res) => res.json());
    dispatch(receiveReport(data));
};

export const createReportThunk = (payload) => async (dispatch) => {
...
export const editReportThunk = (payload) => async (dispatch) => {
